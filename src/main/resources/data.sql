insert into course(id, name) values (10001, 'hibernate' );
insert into course(id, name) values (10002, 'JPA' );
insert into course(id, name) values (10003, 'JDBC' );

insert into student(id, name) values (20001, 'Stefan');
insert into student(id, name) values (20002, 'Petar');
insert into student(id, name) values (20003, 'Ivan');

insert into passport(id, number) values (30001,'A111');
insert into passport(id, number) values (30002,'A222');
insert into passport(id, number) values (30003,'A333');

insert into review(id, rating, description) values (40001, '5', 'Great Course');
insert into review(id, rating, description) values (40002, '4', 'Wonderful Course');
insert into review(id, rating, description) values (40003, '5', 'Awesome Course');